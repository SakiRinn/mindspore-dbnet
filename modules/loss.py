...
class L1BalanceCELoss(nn.LossBase):
    ...
    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5, bce_replace="dice"):
...
class BalanceCrossEntropyLoss(nn.LossBase):
    ...
    def construct(self, pred, gt, mask):
        ...
        negative_loss, _ = self.topk(negative_loss, 16*640*640) # sort the losses in descending order.
...
def test_bce():
    ...
    for _ in range(20):
        ...
        pred_random = Tensor(np.random.rand(1,SHAPE,SHAPE), dtype=ms.float16)
        gt_random = Tensor(np.random.rand(1,SHAPE,SHAPE), dtype=ms.float16)
        mask_random = Tensor(np.random.rand(1,SHAPE,SHAPE), dtype=ms.float16)
...
if __name__ == '__main__':
    ...
    context.set_context(mode=context.GRAPH_MODE, device_id=3)
    compare_loss()
